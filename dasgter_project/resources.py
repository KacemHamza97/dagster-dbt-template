...
from dagster import EnvVar, file_relative_path, get_dagster_logger
...
duckdb_resource = DuckDBResource(
    database=file_relative_path(__file__, "../data/db/duckdb.db")
)
...
class SlingResource(ConfigurableResource):
    ...
    def sync(self, source_table: str, destination_file: str, mode="full-refresh", primary_key=None):
        config = {
            "source": {
        ...
        file_size = 0
        if os.path.isfile(os.path.abspath(destination_file)):
            file_size = os.path.getsize(os.path.abspath(destination_file))
        else:
